.class public final Lcn/com/believer/songyuanframework/openapi/storage/box/impl/simple/utils/ConverterUtils;
.super Ljava/lang/Object;
.source "ConverterUtils.java"


# direct methods
.method private constructor <init>()V
    .registers 1

    .prologue
    .line 29
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    .line 30
    return-void
.end method

.method public static toBox(Lorg/dom4j/Element;)Lcn/com/believer/songyuanframework/openapi/storage/box/objects/Box;
    .registers 6
    .parameter "boxElm"

    .prologue
    .line 251
    invoke-static {}, Lcn/com/believer/songyuanframework/openapi/storage/box/factories/BoxObjectFactory;->createBox()Lcn/com/believer/songyuanframework/openapi/storage/box/objects/Box;

    move-result-object v0

    .line 252
    .local v0, box:Lcn/com/believer/songyuanframework/openapi/storage/box/objects/Box;
    const-string v4, "id"

    invoke-interface {p0, v4}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v1

    .line 253
    .local v1, idElm:Lorg/dom4j/Element;
    const-string v4, "url"

    invoke-interface {p0, v4}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v3

    .line 254
    .local v3, urlElm:Lorg/dom4j/Element;
    const-string v4, "status"

    invoke-interface {p0, v4}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v2

    .line 255
    .local v2, statusElm:Lorg/dom4j/Element;
    if-eqz v1, :cond_1f

    .line 256
    invoke-interface {v1}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v4

    invoke-interface {v0, v4}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/Box;->setId(Ljava/lang/String;)V

    .line 258
    :cond_1f
    if-eqz v3, :cond_28

    .line 259
    invoke-interface {v3}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v4

    invoke-interface {v0, v4}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/Box;->setUrl(Ljava/lang/String;)V

    .line 261
    :cond_28
    if-eqz v2, :cond_31

    .line 262
    invoke-interface {v2}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v4

    invoke-interface {v0, v4}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/Box;->setStatus(Ljava/lang/String;)V

    .line 264
    :cond_31
    return-object v0
.end method

.method public static toBoxFile(Lorg/dom4j/Element;)Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFile;
    .registers 26
    .parameter "infoElm"

    .prologue
    .line 112
    invoke-static {}, Lcn/com/believer/songyuanframework/openapi/storage/box/factories/BoxObjectFactory;->createBoxFile()Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFile;

    move-result-object v2

    .line 114
    .local v2, boxFile:Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFile;
    const-string v24, "file_id"

    move-object/from16 v0, p0

    move-object/from16 v1, v24

    invoke-interface {v0, v1}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v9

    .line 115
    .local v9, fileIdElm:Lorg/dom4j/Element;
    const-string v24, "file_name"

    move-object/from16 v0, p0

    move-object/from16 v1, v24

    invoke-interface {v0, v1}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v10

    .line 116
    .local v10, fileNameElm:Lorg/dom4j/Element;
    const-string v24, "folder_id"

    move-object/from16 v0, p0

    move-object/from16 v1, v24

    invoke-interface {v0, v1}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v11

    .line 117
    .local v11, folderIdElm:Lorg/dom4j/Element;
    const-string v24, "shared"

    move-object/from16 v0, p0

    move-object/from16 v1, v24

    invoke-interface {v0, v1}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v13

    .line 119
    .local v13, sharedElm:Lorg/dom4j/Element;
    const-string v24, "shared_name"

    move-object/from16 v0, p0

    move-object/from16 v1, v24

    invoke-interface {v0, v1}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v14

    .line 120
    .local v14, sharedNameElm:Lorg/dom4j/Element;
    const-string v24, "size"

    move-object/from16 v0, p0

    move-object/from16 v1, v24

    invoke-interface {v0, v1}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v18

    .line 122
    .local v18, sizeElm:Lorg/dom4j/Element;
    const-string v24, "description"

    move-object/from16 v0, p0

    move-object/from16 v1, v24

    invoke-interface {v0, v1}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v7

    .line 123
    .local v7, descriptionElm:Lorg/dom4j/Element;
    const-string v24, "sha1"

    move-object/from16 v0, p0

    move-object/from16 v1, v24

    invoke-interface {v0, v1}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v12

    .line 124
    .local v12, sha1Elm:Lorg/dom4j/Element;
    const-string v24, "created"

    move-object/from16 v0, p0

    move-object/from16 v1, v24

    invoke-interface {v0, v1}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v5

    .line 125
    .local v5, createdElm:Lorg/dom4j/Element;
    const-string v24, "updated"

    move-object/from16 v0, p0

    move-object/from16 v1, v24

    invoke-interface {v0, v1}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v22

    .line 126
    .local v22, updatedElm:Lorg/dom4j/Element;
    if-eqz v9, :cond_73

    .line 127
    invoke-interface {v9}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v24

    move-object/from16 v0, v24

    invoke-interface {v2, v0}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFile;->setFileId(Ljava/lang/String;)V

    .line 129
    :cond_73
    if-eqz v10, :cond_7e

    .line 130
    invoke-interface {v10}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v24

    move-object/from16 v0, v24

    invoke-interface {v2, v0}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFile;->setFileName(Ljava/lang/String;)V

    .line 132
    :cond_7e
    if-eqz v11, :cond_89

    .line 133
    invoke-interface {v11}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v24

    move-object/from16 v0, v24

    invoke-interface {v2, v0}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFile;->setFolderId(Ljava/lang/String;)V

    .line 135
    :cond_89
    if-eqz v13, :cond_a0

    .line 136
    invoke-interface {v13}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v15

    .line 137
    .local v15, sharedStr:Ljava/lang/String;
    const-string v24, "0"

    move-object/from16 v0, v24

    invoke-virtual {v0, v15}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v24

    if-eqz v24, :cond_f3

    .line 138
    const/16 v24, 0x0

    move/from16 v0, v24

    invoke-interface {v2, v0}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFile;->setShared(Z)V

    .line 143
    .end local v15           #sharedStr:Ljava/lang/String;
    :cond_a0
    :goto_a0
    if-eqz v14, :cond_ab

    .line 144
    invoke-interface {v14}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v24

    move-object/from16 v0, v24

    invoke-interface {v2, v0}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFile;->setSharedName(Ljava/lang/String;)V

    .line 146
    :cond_ab
    if-eqz v18, :cond_bc

    .line 147
    const-wide/16 v16, 0x0

    .line 148
    .local v16, size:J
    invoke-interface/range {v18 .. v18}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v19

    .line 150
    .local v19, sizeStr:Ljava/lang/String;
    :try_start_b3
    invoke-static/range {v19 .. v19}, Ljava/lang/Long;->parseLong(Ljava/lang/String;)J
    :try_end_b6
    .catch Ljava/lang/NumberFormatException; {:try_start_b3 .. :try_end_b6} :catch_fb

    move-result-wide v16

    .line 154
    :goto_b7
    move-wide/from16 v0, v16

    invoke-interface {v2, v0, v1}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFile;->setSize(J)V

    .line 156
    .end local v16           #size:J
    .end local v19           #sizeStr:Ljava/lang/String;
    :cond_bc
    if-eqz v7, :cond_c7

    .line 157
    invoke-interface {v7}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v24

    move-object/from16 v0, v24

    invoke-interface {v2, v0}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFile;->setDescription(Ljava/lang/String;)V

    .line 159
    :cond_c7
    if-eqz v12, :cond_d2

    .line 160
    invoke-interface {v12}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v24

    move-object/from16 v0, v24

    invoke-interface {v2, v0}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFile;->setSha1(Ljava/lang/String;)V

    .line 162
    :cond_d2
    if-eqz v5, :cond_e1

    .line 163
    const-wide/16 v3, 0x0

    .line 164
    .local v3, created:J
    invoke-interface {v5}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v6

    .line 166
    .local v6, createdStr:Ljava/lang/String;
    :try_start_da
    invoke-static {v6}, Ljava/lang/Long;->parseLong(Ljava/lang/String;)J
    :try_end_dd
    .catch Ljava/lang/NumberFormatException; {:try_start_da .. :try_end_dd} :catch_ff

    move-result-wide v3

    .line 170
    :goto_de
    invoke-interface {v2, v3, v4}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFile;->setCreated(J)V

    .line 172
    .end local v3           #created:J
    .end local v6           #createdStr:Ljava/lang/String;
    :cond_e1
    if-eqz v22, :cond_f2

    .line 173
    const-wide/16 v20, 0x0

    .line 174
    .local v20, updated:J
    invoke-interface/range {v22 .. v22}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v23

    .line 176
    .local v23, updatedStr:Ljava/lang/String;
    :try_start_e9
    invoke-static/range {v23 .. v23}, Ljava/lang/Long;->parseLong(Ljava/lang/String;)J
    :try_end_ec
    .catch Ljava/lang/NumberFormatException; {:try_start_e9 .. :try_end_ec} :catch_103

    move-result-wide v20

    .line 180
    :goto_ed
    move-wide/from16 v0, v20

    invoke-interface {v2, v0, v1}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFile;->setUpdated(J)V

    .line 183
    .end local v20           #updated:J
    .end local v23           #updatedStr:Ljava/lang/String;
    :cond_f2
    return-object v2

    .line 140
    .restart local v15       #sharedStr:Ljava/lang/String;
    :cond_f3
    const/16 v24, 0x1

    move/from16 v0, v24

    invoke-interface {v2, v0}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFile;->setShared(Z)V

    goto :goto_a0

    .line 151
    .end local v15           #sharedStr:Ljava/lang/String;
    .restart local v16       #size:J
    .restart local v19       #sizeStr:Ljava/lang/String;
    :catch_fb
    move-exception v8

    .line 152
    .local v8, e:Ljava/lang/NumberFormatException;
    const-wide/high16 v16, -0x8000

    goto :goto_b7

    .line 167
    .end local v8           #e:Ljava/lang/NumberFormatException;
    .end local v16           #size:J
    .end local v19           #sizeStr:Ljava/lang/String;
    .restart local v3       #created:J
    .restart local v6       #createdStr:Ljava/lang/String;
    :catch_ff
    move-exception v8

    .line 168
    .restart local v8       #e:Ljava/lang/NumberFormatException;
    const-wide/high16 v3, -0x8000

    goto :goto_de

    .line 177
    .end local v3           #created:J
    .end local v6           #createdStr:Ljava/lang/String;
    .end local v8           #e:Ljava/lang/NumberFormatException;
    .restart local v20       #updated:J
    .restart local v23       #updatedStr:Ljava/lang/String;
    :catch_103
    move-exception v8

    .line 178
    .restart local v8       #e:Ljava/lang/NumberFormatException;
    const-wide/high16 v20, -0x8000

    goto :goto_ed
.end method

.method public static toBoxFolder(Lorg/dom4j/Element;)Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFolder;
    .registers 13
    .parameter "folderElm"

    .prologue
    .line 60
    invoke-static {}, Lcn/com/believer/songyuanframework/openapi/storage/box/factories/BoxObjectFactory;->createBoxFolder()Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFolder;

    move-result-object v0

    .line 62
    .local v0, boxFolder:Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFolder;
    const-string v11, "folder_id"

    invoke-interface {p0, v11}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v1

    .line 64
    .local v1, folderIdElm:Lorg/dom4j/Element;
    const-string v11, "folder_name"

    invoke-interface {p0, v11}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v2

    .line 66
    .local v2, folderNameElm:Lorg/dom4j/Element;
    const-string v11, "folder_type_id"

    invoke-interface {p0, v11}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v3

    .line 68
    .local v3, folderTypeIdElm:Lorg/dom4j/Element;
    const-string v11, "parent_folder_id"

    invoke-interface {p0, v11}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v4

    .line 69
    .local v4, parentFolderIdElm:Lorg/dom4j/Element;
    const-string v11, "user_id"

    invoke-interface {p0, v11}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v10

    .line 70
    .local v10, userIdElm:Lorg/dom4j/Element;
    const-string v11, "path"

    invoke-interface {p0, v11}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v6

    .line 71
    .local v6, pathElm:Lorg/dom4j/Element;
    const-string v11, "shared"

    invoke-interface {p0, v11}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v8

    .line 73
    .local v8, sharedElm:Lorg/dom4j/Element;
    const-string v11, "public_name"

    invoke-interface {p0, v11}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v7

    .line 75
    .local v7, publicNameElm:Lorg/dom4j/Element;
    const-string v11, "show_comments"

    invoke-interface {p0, v11}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v9

    .line 77
    .local v9, showCommentsElm:Lorg/dom4j/Element;
    const-string v11, "password"

    invoke-interface {p0, v11}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v5

    .line 79
    .local v5, passwordElm:Lorg/dom4j/Element;
    invoke-interface {v1}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v11

    invoke-interface {v0, v11}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFolder;->setFolderId(Ljava/lang/String;)V

    .line 80
    invoke-interface {v2}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v11

    invoke-interface {v0, v11}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFolder;->setFolderName(Ljava/lang/String;)V

    .line 81
    if-eqz v3, :cond_57

    .line 82
    invoke-interface {v3}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v11

    invoke-interface {v0, v11}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFolder;->setFolderTypeId(Ljava/lang/String;)V

    .line 84
    :cond_57
    invoke-interface {v4}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v11

    invoke-interface {v0, v11}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFolder;->setParentFolderId(Ljava/lang/String;)V

    .line 85
    if-eqz v5, :cond_67

    .line 86
    invoke-interface {v5}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v11

    invoke-interface {v0, v11}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFolder;->setPassword(Ljava/lang/String;)V

    .line 88
    :cond_67
    if-eqz v6, :cond_70

    .line 89
    invoke-interface {v6}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v11

    invoke-interface {v0, v11}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFolder;->setPath(Ljava/lang/String;)V

    .line 91
    :cond_70
    if-eqz v7, :cond_79

    .line 92
    invoke-interface {v7}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v11

    invoke-interface {v0, v11}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFolder;->setPublicName(Ljava/lang/String;)V

    .line 94
    :cond_79
    if-eqz v8, :cond_82

    .line 95
    invoke-interface {v8}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v11

    invoke-interface {v0, v11}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFolder;->setShared(Ljava/lang/String;)V

    .line 97
    :cond_82
    if-eqz v9, :cond_8b

    .line 98
    invoke-interface {v9}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v11

    invoke-interface {v0, v11}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFolder;->setShowComments(Ljava/lang/String;)V

    .line 100
    :cond_8b
    if-eqz v10, :cond_94

    .line 101
    invoke-interface {v10}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v11

    invoke-interface {v0, v11}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFolder;->setUserId(Ljava/lang/String;)V

    .line 103
    :cond_94
    return-object v0
.end method

.method public static toFriendsList(Lorg/dom4j/Element;)Ljava/util/List;
    .registers 20
    .parameter "friendsElm"

    .prologue
    .line 192
    new-instance v10, Ljava/util/ArrayList;

    invoke-direct {v10}, Ljava/util/ArrayList;-><init>()V

    .line 193
    .local v10, friendsList:Ljava/util/List;
    if-eqz p0, :cond_10

    .line 194
    const/4 v11, 0x0

    .local v11, i:I
    :goto_8
    invoke-interface/range {p0 .. p0}, Lorg/dom4j/Element;->nodeCount()I

    move-result v18

    move/from16 v0, v18

    if-lt v11, v0, :cond_11

    .line 241
    .end local v11           #i:I
    :cond_10
    return-object v10

    .line 195
    .restart local v11       #i:I
    :cond_11
    invoke-static {}, Lcn/com/believer/songyuanframework/openapi/storage/box/factories/BoxObjectFactory;->createBoxFriend()Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFriend;

    move-result-object v8

    .line 196
    .local v8, friend:Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFriend;
    move-object/from16 v0, p0

    invoke-interface {v0, v11}, Lorg/dom4j/Element;->node(I)Lorg/dom4j/Node;

    move-result-object v9

    check-cast v9, Lorg/dom4j/Element;

    .line 198
    .local v9, friendElm:Lorg/dom4j/Element;
    const-string v18, "name"

    move-object/from16 v0, v18

    invoke-interface {v9, v0}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v13

    .line 199
    .local v13, nameElm:Lorg/dom4j/Element;
    const-string v18, "email"

    move-object/from16 v0, v18

    invoke-interface {v9, v0}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v7

    .line 200
    .local v7, emailElm:Lorg/dom4j/Element;
    const-string v18, "accepted"

    move-object/from16 v0, v18

    invoke-interface {v9, v0}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v1

    .line 201
    .local v1, acceptedElm:Lorg/dom4j/Element;
    const-string v18, "avatar_url"

    move-object/from16 v0, v18

    invoke-interface {v9, v0}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v2

    .line 202
    .local v2, avatarUrlElm:Lorg/dom4j/Element;
    const-string v18, "boxes"

    move-object/from16 v0, v18

    invoke-interface {v9, v0}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v6

    .line 203
    .local v6, boxesElm:Lorg/dom4j/Element;
    const-string v18, "subscriptions"

    move-object/from16 v0, v18

    invoke-interface {v9, v0}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v17

    .line 205
    .local v17, subscriptionsElm:Lorg/dom4j/Element;
    if-eqz v13, :cond_58

    .line 206
    invoke-interface {v13}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v18

    move-object/from16 v0, v18

    invoke-interface {v8, v0}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFriend;->setName(Ljava/lang/String;)V

    .line 208
    :cond_58
    if-eqz v7, :cond_63

    .line 209
    invoke-interface {v7}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v18

    move-object/from16 v0, v18

    invoke-interface {v8, v0}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFriend;->setEmail(Ljava/lang/String;)V

    .line 211
    :cond_63
    if-eqz v1, :cond_6e

    .line 212
    invoke-interface {v1}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v18

    move-object/from16 v0, v18

    invoke-interface {v8, v0}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFriend;->setAccepted(Ljava/lang/String;)V

    .line 214
    :cond_6e
    if-eqz v2, :cond_79

    .line 215
    invoke-interface {v2}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v18

    move-object/from16 v0, v18

    invoke-interface {v8, v0}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFriend;->setAvatarUrl(Ljava/lang/String;)V

    .line 217
    :cond_79
    new-instance v5, Ljava/util/ArrayList;

    invoke-direct {v5}, Ljava/util/ArrayList;-><init>()V

    .line 218
    .local v5, boxList:Ljava/util/List;
    if-eqz v6, :cond_89

    .line 219
    const/4 v12, 0x0

    .local v12, j:I
    :goto_81
    invoke-interface {v6}, Lorg/dom4j/Element;->nodeCount()I

    move-result v18

    move/from16 v0, v18

    if-lt v12, v0, :cond_a8

    .line 225
    .end local v12           #j:I
    :cond_89
    invoke-interface {v8, v5}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFriend;->setBoxes(Ljava/util/List;)V

    .line 227
    new-instance v16, Ljava/util/ArrayList;

    invoke-direct/range {v16 .. v16}, Ljava/util/ArrayList;-><init>()V

    .line 228
    .local v16, subscriptionList:Ljava/util/List;
    if-eqz v17, :cond_9c

    .line 229
    const/4 v12, 0x0

    .restart local v12       #j:I
    :goto_94
    invoke-interface/range {v17 .. v17}, Lorg/dom4j/Element;->nodeCount()I

    move-result v18

    move/from16 v0, v18

    if-lt v12, v0, :cond_b8

    .line 236
    .end local v12           #j:I
    :cond_9c
    move-object/from16 v0, v16

    invoke-interface {v8, v0}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxFriend;->setSubscriptions(Ljava/util/List;)V

    .line 238
    invoke-interface {v10, v8}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    .line 194
    add-int/lit8 v11, v11, 0x1

    goto/16 :goto_8

    .line 220
    .end local v16           #subscriptionList:Ljava/util/List;
    .restart local v12       #j:I
    :cond_a8
    invoke-interface {v6, v12}, Lorg/dom4j/Element;->node(I)Lorg/dom4j/Node;

    move-result-object v4

    check-cast v4, Lorg/dom4j/Element;

    .line 221
    .local v4, boxElm:Lorg/dom4j/Element;
    invoke-static {v4}, Lcn/com/believer/songyuanframework/openapi/storage/box/impl/simple/utils/ConverterUtils;->toBox(Lorg/dom4j/Element;)Lcn/com/believer/songyuanframework/openapi/storage/box/objects/Box;

    move-result-object v3

    .line 222
    .local v3, box:Lcn/com/believer/songyuanframework/openapi/storage/box/objects/Box;
    invoke-interface {v5, v3}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    .line 219
    add-int/lit8 v12, v12, 0x1

    goto :goto_81

    .line 231
    .end local v3           #box:Lcn/com/believer/songyuanframework/openapi/storage/box/objects/Box;
    .end local v4           #boxElm:Lorg/dom4j/Element;
    .restart local v16       #subscriptionList:Ljava/util/List;
    :cond_b8
    move-object/from16 v0, v17

    invoke-interface {v0, v12}, Lorg/dom4j/Element;->node(I)Lorg/dom4j/Node;

    move-result-object v15

    .line 230
    check-cast v15, Lorg/dom4j/Element;

    .line 232
    .local v15, subscriptionElm:Lorg/dom4j/Element;
    invoke-static {v15}, Lcn/com/believer/songyuanframework/openapi/storage/box/impl/simple/utils/ConverterUtils;->toSubscription(Lorg/dom4j/Element;)Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxSubscription;

    move-result-object v14

    .line 233
    .local v14, subscription:Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxSubscription;
    move-object/from16 v0, v16

    invoke-interface {v0, v14}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    .line 229
    add-int/lit8 v12, v12, 0x1

    goto :goto_94
.end method

.method public static toSubscription(Lorg/dom4j/Element;)Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxSubscription;
    .registers 7
    .parameter "subscriptionElm"

    .prologue
    .line 274
    invoke-static {}, Lcn/com/believer/songyuanframework/openapi/storage/box/factories/BoxObjectFactory;->createBoxSubscription()Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxSubscription;

    move-result-object v2

    .line 275
    .local v2, subscription:Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxSubscription;
    const-string v5, "box_id"

    invoke-interface {p0, v5}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v0

    .line 276
    .local v0, boxIdElm:Lorg/dom4j/Element;
    const-string v5, "user_name"

    invoke-interface {p0, v5}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v4

    .line 277
    .local v4, userNameElm:Lorg/dom4j/Element;
    const-string v5, "url"

    invoke-interface {p0, v5}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v3

    .line 278
    .local v3, urlElm:Lorg/dom4j/Element;
    const-string v5, "status"

    invoke-interface {p0, v5}, Lorg/dom4j/Element;->element(Ljava/lang/String;)Lorg/dom4j/Element;

    move-result-object v1

    .line 279
    .local v1, statusElm:Lorg/dom4j/Element;
    if-eqz v0, :cond_25

    .line 280
    invoke-interface {v0}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v5

    invoke-interface {v2, v5}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxSubscription;->setBoxId(Ljava/lang/String;)V

    .line 282
    :cond_25
    if-eqz v4, :cond_2e

    .line 283
    invoke-interface {v4}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v5

    invoke-interface {v2, v5}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxSubscription;->setUserName(Ljava/lang/String;)V

    .line 285
    :cond_2e
    if-eqz v3, :cond_37

    .line 286
    invoke-interface {v3}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v5

    invoke-interface {v2, v5}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxSubscription;->setUrl(Ljava/lang/String;)V

    .line 288
    :cond_37
    if-eqz v1, :cond_40

    .line 289
    invoke-interface {v1}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v5

    invoke-interface {v2, v5}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxSubscription;->setStatus(Ljava/lang/String;)V

    .line 291
    :cond_40
    return-object v2
.end method

.method public static transferTags2List(Lorg/dom4j/Element;)Ljava/util/List;
    .registers 8
    .parameter "tagsElm"

    .prologue
    .line 40
    new-instance v5, Ljava/util/ArrayList;

    invoke-direct {v5}, Ljava/util/ArrayList;-><init>()V

    .line 41
    .local v5, tagsList:Ljava/util/List;
    const/4 v0, 0x0

    .local v0, i:I
    :goto_6
    invoke-interface {p0}, Lorg/dom4j/Element;->nodeCount()I

    move-result v6

    if-lt v0, v6, :cond_d

    .line 50
    return-object v5

    .line 42
    :cond_d
    invoke-interface {p0, v0}, Lorg/dom4j/Element;->node(I)Lorg/dom4j/Node;

    move-result-object v4

    check-cast v4, Lorg/dom4j/Element;

    .line 43
    .local v4, tagElm:Lorg/dom4j/Element;
    const-string v6, "id"

    invoke-interface {v4, v6}, Lorg/dom4j/Element;->attributeValue(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    .line 44
    .local v1, id:Ljava/lang/String;
    invoke-interface {v4}, Lorg/dom4j/Element;->getText()Ljava/lang/String;

    move-result-object v2

    .line 45
    .local v2, name:Ljava/lang/String;
    invoke-static {}, Lcn/com/believer/songyuanframework/openapi/storage/box/factories/BoxObjectFactory;->createBoxTag()Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxTag;

    move-result-object v3

    .line 46
    .local v3, tag:Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxTag;
    invoke-interface {v3, v1}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxTag;->setId(Ljava/lang/String;)V

    .line 47
    invoke-interface {v3, v2}, Lcn/com/believer/songyuanframework/openapi/storage/box/objects/BoxTag;->setName(Ljava/lang/String;)V

    .line 48
    invoke-interface {v5, v3}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    .line 41
    add-int/lit8 v0, v0, 0x1

    goto :goto_6
.end method
